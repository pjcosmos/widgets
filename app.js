...
function renderTodos(){
  const items = load().filter(t => !t.done).sort((a,b)=> (a.date||"").localeCompare(b.date) || a.createdAt-b.createdAt);
  listEl.innerHTML = "";
...
function drawCalendar(){
  ...
  const tasksByDate = {};
  for (const t of load()){ // 완료 포함 전체 기록
    if (!t.date) continue;
